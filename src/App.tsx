...
const HomePage = () => {
  return (
    <div className="text-center py-10 px-4 sm:px-6 md:px-10 lg:px-20">
      ...
      <h1 className="mt-4 text-xl sm:text-2xl font-semibold">PLEASE DONATE YOUR AMOUNT</h1>
      <h2 className="mt-2 text-lg sm:text-xl font-semibold">Most Trusted Value List</h2>
      <h3 className="mt-1 text-md sm:text-lg font-semibold">Blox Fruits Values</h3>
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center gap-6 mt-10">
        {images.map((img, index) => (
          <motion.img
            key={index}
            src={img}
            alt={`Blox Fruit ${index + 1}`}
            className="w-24 h-24 sm:w-32 sm:h-32 rounded-lg border-4 border-blue-500 object-cover"
      ...
      <div className="flex justify-center gap-3 sm:gap-5 mt-5 flex-wrap px-2">
        {["Common", "Uncommon", "Rare", "Legendary", "Mythical", "Gamepass"].map((category, index) => (
          <motion.div
            key={index}
            className="px-3 sm:px-5 py-2 sm:py-3 bg-gray-700 rounded-lg text-center text-white text-sm sm:text-lg shadow-lg cursor-pointer"
